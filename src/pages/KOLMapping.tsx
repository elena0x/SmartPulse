...
import { MapPin, Upload, Tags, Search, X, FileSpreadsheet, CheckCircle2, UploadCloud, UserPlus, Database, Loader2 } from "lucide-react";
...
import { useHCPProfiles, useInsertHCPs, HCPInsert } from "@/hooks/useHCPProfiles";
// Note: This page redirects to /hcp-list in App.tsx
...
const KOLMapping = () => {
  ...
  /* Upload mode */
  const [parsedRows, setParsedRows] = useState<HCPInsert[]>([]);
  ...
  const insertMutation = useInsertHCPs();
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const json = await parseSpreadsheet(file);
        ...
        setPreviewCols(cols);
        setPreviewData(json.slice(0, 100)); // Show first 100 rows for preview
        ...
        cols.forEach((col) => {
          ...
          for (const [field, keywords] of Object.entries(mappingHints)) {
  ...
  /* ── Build records from column mapping ── */
  const buildRecords = (): HCPInsert[] => {
    if (previewData.length === 0) return [];

    const fullData = previewData; // In production could re-read full file
    let autoIdCounter = 1;

    return fullData.map((row) => {
      const record: any = { is_subscribed: true };
      for (const [col, field] of Object.entries(colMapping)) {
        if (field) record[field] = (row[col] || "").toString().trim();
      }
      // Generate hcp_id if not mapped
      if (!record.hcp_id || !record.hcp_id.trim()) {
        record.hcp_id = `HCP-UPLOAD-${Date.now()}-${autoIdCounter++}`;
      }
      // Name is required
      if (!record.name || !record.name.trim()) {
        record.name = "未知";
      }
      return record as HCPInsert;
    });
  };
  ...
  const handleConfirmUpload = () => {
    ...
    if (records.length === 0) {
      toast({ title: "无数据", description: "请先上传文件并配置列映射" });
      return;
    }
    ...
    insertMutation.mutate(records, {
      onSuccess: (data) => {
        toast({
          title: "导入成功",
          description: `成功导入 ${data.length} 条 HCP 记录到数据库`,
        });
        setParsedRows([]);
        setPreviewData([]);
        setPreviewCols([]);
      ...
      <div className="max-w-[1400px] mx-auto">
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <h1 className="text-lg font-bold text-foreground">数据管理</h1>
              <p className="text-xs text-muted-foreground">通过上传 Excel 导入 HCP 数据，或按标签检索已有数据</p>
            </div>
            <div className="ml-auto flex items-center gap-2 text-xs text-muted-foreground">
              <Database className="w-3.5 h-3.5" />
              数据库共 {allHCPs.length} 条记录
            </div>
          </div>
                ...
                <div className="card-elevated rounded-xl p-5 space-y-4">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                    {previewCols.map((col) => (
                      <div key={col} className="space-y-1">
                        <label className="text-xs font-medium text-muted-foreground truncate block" title={col}>
                          {col}
                        </label>
                        <Select
                          value={colMapping[col] || ""}
                          onValueChange={(v) =>
                            setColMapping((prev) => ({ ...prev, [col]: v === "__none__" ? "" : v }))
                          }
                        >
                          ...
                          <SelectContent>
                            ...
                            {FIELD_OPTIONS.map((opt) => (
                              <SelectItem
                                key={opt.value}
                                value={opt.value}
                                disabled={usedFields.has(opt.value) && colMapping[col] !== opt.value}
                              >
                                {opt.label}
                              </SelectItem>
                            ))}
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <Button
                      ...
                      disabled={insertMutation.isPending || Object.values(colMapping).filter(Boolean).length === 0}
                      className="gap-1.5"
                      size="sm"
                    >
                    ...
                    <Table>
                      <TableHeader>
                        <TableRow className="bg-muted/60">
                          ...
                          <TableHead className="text-xs font-semibold whitespace-nowrap">性别</TableHead>
                          <TableHead className="text-xs font-semibold whitespace-nowrap">省份</TableHead>
                          <TableHead className="text-xs font-semibold whitespace-nowrap">城市</TableHead>
                          <TableHead className="text-xs font-semibold whitespace-nowrap">机构名称</TableHead>
                          <TableHead className="text-xs font-semibold whitespace-nowrap">医院分类</TableHead>
                      ...
                      <TableBody>
                        {tagResults.map((hcp) => (
                          <TableRow key={hcp.hcp_id} className="hover:bg-primary/5 transition-colors">
                            ...
                            <TableCell className="text-xs text-muted-foreground">{hcp.gender}</TableCell>
                            <TableCell className="text-xs text-muted-foreground whitespace-nowrap">{hcp.province}</TableCell>
                            <TableCell className="text-xs text-muted-foreground whitespace-nowrap">{hcp.city}</TableCell>
                            <TableCell className="text-xs text-card-foreground whitespace-nowrap">{hcp.institution}</TableCell>
                            <TableCell>
                              <Badge variant="secondary" className="text-[10px] font-normal">{hcp.hospital_category}</Badge>
                            </TableCell>
